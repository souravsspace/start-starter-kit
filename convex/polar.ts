...
export const polar = new Polar(components.polar, {
  ...
  getUserInfo: async (ctx): Promise<{ userId: string; email: string }> => {
    ...
    if (!user || !user.email) {
      throw new Error(
        "User not authenticated for Polar checkout. Please log in first.",
      );
    }
...
export const getSubscriptionStatus = query({
  handler: async (ctx) => {
    ...
    let currentPlan = determineCurrentPlan(subscription?.productKey);

    const { canUpgradeTo, canDowngradeTo } =
      getAvailablePlanChanges(currentPlan, subscription);
 ...
 */
function getAvailablePlanChanges(currentPlan: SubscriptionPlan, subscription?: any) {
  const canUpgradeTo: SubscriptionPlan[] = [];
  const canDowngradeTo: SubscriptionPlan[] = [];

  // Don't allow changes if subscription is already marked for cancellation
  ...
  switch (currentPlan) {
    case "starter":
      ...
      canUpgradeTo.push("professional");
      break;
    case "professional":
      // Can downgrade to starter
...
export const createCheckout = mutation({
  ...
  handler: async (
    ...
    return {
      message:
        "Checkout validation successful. Use the generateCheckoutLink action to get the URL.",
      currentPlan,
...
function validatePlanChange(
  ...
  const upgrades = [{ from: "starter", to: "professional" }];

  // Check if this is a downgrade
  const downgrades = [{ from: "professional", to: "starter" }];

  const isUpgrade = upgrades.some(
    (up) => up.from === currentPlan && up.to === targetPlan,
  );
  const isDowngrade = downgrades.some(
    (down) => down.from === currentPlan && down.to === targetPlan,
  );
...
export const getSubscriptionDetails = query({
  handler: async (ctx) => {
    try {
      ...
      let isPremium = false;
      let productKey = subscription?.productKey;

      if (subscription) {
        // Determine premium status based on product key
        isPremium = productKey === plans.PROFESSIONAL_MONTHLY.name;
      }
