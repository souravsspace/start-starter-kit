				...
				>
					<div className="relative flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
						<div className="flex w-full justify-between lg:w-auto">
							...
							<button
								onClick={() => {
									trackButtonClick("header_mobile_menu_toggle", {
										state: menuState ? "closing" : "opening",
									});
								...
								{menuItems.map((item, index) => (
									<li key={index}>
										<Link
											...
											onClick={() =>
												trackButtonClick("header_navigation_link", {
													item: item.name,
													href: item.href,
												})
											}
									...
									{menuItems.map((item, index) => (
										<li key={item.href}>
											<Link
												...
												className="text-muted-foreground hover:text-accent-foreground block duration-150"
												onClick={() =>
													trackButtonClick("header_mobile_navigation_link", {
														item: item.name,
														href: item.href,
													})
												}
							...
							<div className="flex w-full flex-col space-y-3 sm:flex-row sm:gap-3 sm:space-y-0 md:w-fit">
								{isLoggedIn ? (
									...
									>
										<Link
											onClick={() =>
												trackButtonClick("header_dashboard_button")
											}
											to="/dashboard"
										>
											<span>Dashboard</span>
									...
									<>
										...
										>
											<Link
												to="/auth/login"
												onClick={() => trackButtonClick("header_login_button")}
											>
												<span>Login</span>
											</Link>
										</Button>
										...
										>
											<Link
												to="/auth/register"
												onClick={() => trackButtonClick("header_signup_button")}
											>
												<span>Sign Up</span>
											</Link>
										</Button>
								...
								>
									<Link
										...
										onClick={() =>
											trackButtonClick("header_primary_cta", {
