...
function RouteComponent() {
  ...
  const handleCheckout = async (plan: "professional") => {
    ...
    setLoadingPlan(plan);
    try {
      // Use configured product IDs from plans object
      const productIds = [plans.PROFESSIONAL_MONTHLY.id];
  ...
  const getButtonState = (plan: "starter" | "professional") => {
    if (!subscriptionStatus?.isAuthenticated) {
      return { disabled: false, text: "Get Started" };
    }

    // Disable all buttons if subscription is already cancelled
    if (subscriptionDetails?.cancelAtPeriodEnd) {
      return { disabled: true, text: "Cancellation Pending" };
    }
    ...
    if (subscriptionStatus.canDowngradeTo.includes(plan)) {
      return { disabled: false, text: "Downgrade" };
    }
